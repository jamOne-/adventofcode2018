...
def tick(board, units):
  for position in sorted(units.keys(), key=lambda position: (position[1], position[0])):
    ...
    if len(neighbouring_enemies) > 0:
      ...
      weakest_enemy = Unit(weakest_enemy.enemy, weakest_enemy.position, weakest_enemy.hp - ATTACK_POWER)
...
def solve(puzzle_input):
  board, units = parse_input(puzzle_input)
  rounds = 0

  while any(map(lambda unit: not unit.enemy, units.values())):
    tick(board, units)
    rounds += 1
  ...
  return (rounds - 1) * sum(map(lambda unit: unit.hp, units.values()))
